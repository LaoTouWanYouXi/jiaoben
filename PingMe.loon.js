...
http-request ^https:\/\/api\.pingmeapp\.net\/app\/queryBalanceAndBonus script-path=https://raw.githubusercontent.com/LaoTouWanYouXi/jiaoben/main/PingMe.loon.js, timeout=60, tag=PingMe抓包

# PingMe 自动签到 + 视频奖励（每4小时运行一次）
cron "20 0/6 * * *" script-path=https://raw.githubusercontent.com/LaoTouWanYouXi/jiaoben/main/PingMe.loon.js, tag=PingMe签到, wake-system=1
...
if (typeof $request !== 'undefined' && $request) {
  ...
  if (existed) {
    console.log(`【${scriptName}】账号 ${fp} 已存在，跳过重复抓包`);
    $done({});
    return;
  }
  ...
  const alias = `账号${store.order.length + 1}`;
